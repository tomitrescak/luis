...
export class Story extends TestGroup {
  ...
  constructor(parent: TestGroup, name: string, props: StoryConfig) { 
    ...
    this.component = props.component || props.componentWithData && ((props.componentWithData() as any).component || props.componentWithData());
    if (!this.component) {
      throw new Error(`Story "${name}" needs to either define a "get component()" or "componentWithData()"`);
    }
