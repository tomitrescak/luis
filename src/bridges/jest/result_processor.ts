...
module.exports = function(testData: jest.AggregatedResult) {
  ...
  let savePath = path.join(process.env.JEST_ROOT_OUTPUT_PATH || rootDir, 'summary.ts');
  let clone = { ...testData };

  delete clone.startTime;
  clone.testResults.forEach(t => delete t.perfStats);

  let report = JSON.stringify(clone, null, 2);
  ...
  testData.testResults.forEach(suite => {
    ...
    let testFileName = path.basename(suite.testFilePath);
    ...
    try {
      ...
      for (let file of files) {
        if (file.match(new RegExp(testFileName + '\\.snap$'))) {
